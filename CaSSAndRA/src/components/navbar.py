...
navbar = dbc.Navbar(
    [
        ...
        dbc.Nav(dbc.NavItem([
            ...
            dbc.Button(id=ids.BUTTONOPENMODALINFO, n_clicks=0, size='lg',class_name='bi bi-info-square', title='open infobox')
        ]), className='order-md-2 flex-grow-1 align-items-end justify-content-end'),
        dbc.Collapse(
            dbc.Nav([
                dbc.NavItem(dbc.NavLink('State', href='/', active='exact')),
                dbc.NavItem(dbc.NavLink('Mapping', href='/mapping', active='partial')),
                #dbc.NavItem(dbc.NavLink('Taskplanner', href='/taskplanner', active='partial')),
                dbc.NavItem(dbc.NavLink('Settings', href='/settings', active='partial')),
